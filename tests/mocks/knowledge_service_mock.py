...
def mock_knowledge_service(mocker, mock_neo4j_session_factory):
    ...
    mock.create_empty_course = mocker.MagicMock(
        side_effect=instance.create_empty_course
    )
    mock.add_document_to_course = mocker.MagicMock(
        side_effect=instance.add_document_to_course
    )
    mock.set_course_instructors = mocker.MagicMock(
        side_effect=instance.set_course_instructors
    )
    mock.set_course_students = mocker.MagicMock(
        side_effect=instance.set_course_students
    )
    ...
    mock.update_relationship = mocker.MagicMock(
        side_effect=instance.update_relationship
    )
    mock.delete_relationship = mocker.MagicMock(
        side_effect=instance.delete_relationship
    )
    return mock
